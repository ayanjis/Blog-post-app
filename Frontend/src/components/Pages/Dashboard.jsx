...
//       isOnline = onlineUsers.includes(respose.data[0].postCreator?._id);
//       // console.log("dash", respose.data[0].postCreator?._id);
//     } catch (error) {
...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Initialize online status based on fetched posts
      const initialStatus = response.data.reduce((status, post) => {
        status[post.postCreator._id] = onlineUsers.includes(
  ...
  useEffect(() => {
    const updatedStatus = posts.reduce((status, post) => {
      status[post.postCreator._id] = onlineUsers.includes(post.postCreator._id);
      return status;
    }, {});
          ...
          {posts?.map((post) =>
            isLoading ? (
              <div
                key={post._id}
                role="status"
                className="max-w-sm animate-pulse gap-2 w-[260px] h-[160px] p-4 bg-gray-600 bg-opacity-50 rounded-xl border border-gray-600 flex flex-col justify-between max-[425px]:w-full"
              >
                <div className="flex items-center gap-2">
                  <span className="bg-gray-300 rounded-full w-12 h-12"></span>
                  <h3 className="h-5 bg-gray-300 rounded-full w-20"></h3>
              ...
              <div
                key={post._id}
                className="text-white hover:text-white w-[260px] h-[160px] transition ease-in-out hover:scale-[1.05] p-4 gap-2 bg-gray-600 bg-opacity-50 rounded-xl border border-gray-600 flex flex-col justify-between hover:bg-teal-500 max-[425px]:w-full"
              >
                <div className="flex items-center justify-between w-full">
                  <NavLink
                    to={`/user/userinfo/${post.postCreator._id}`}
